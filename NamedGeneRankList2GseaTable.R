...
NamedGeneRankList2GseaTable <- function(rankedgenes, geneset = c("all", "bloodmodules", "MSigDB"), output_directory = getwd(), filename_prefix = "myFile", minSize = minSize, scoreType = c("std", "pos", "neg"), fixed_seed = TRUE, ...) {
  ...
  for(i in allGeneSets){
    ...
    fgseaRes[[i]] <- fgseaMultilevel(pathways=myModuleList[[i]], stats=rankedgenes, sampleSize = 101, minSize = minSize, maxSize = Inf, eps = 0, scoreType = scoreType) %>%
      as_tibble() %>%
      arrange(desc(NES)) %>% 
