...
#Description
#This function takes a named ranked list of genes and applies the fgseaMultLevel function using multiple gene sets relevant to immunology and blood transcriptomics.
...
NamedGeneRankList2GseaTable <- function(rankedgenes, geneset = c("all", "bloodmodules", "MSigDB"), output_directory = getwd(), filename_prefix = "myFile", fixed_seed = TRUE, ...) {
  ...
  for(i in allGeneSets){
    ...
    fgseaRes[[i]] <- fgseaMultilevel(pathways=myModuleList[[i]], stats=rankedgenes, sampleSize = 101, minSize = 20, maxSize = Inf, eps = 0, scoreType = "std") %>%
      as_tibble() %>%
      arrange(desc(NES)) %>% 
